...
export const Avatar: React.FC<AvatarProps> = ({ config, className = '' }) => {
    ...
    return (
        <div className={`relative shrink-0 ${className}`}>
            <div
                className={`${sizeClass} ${backgroundColor} ${textColor} rounded-full flex items-center justify-center overflow-hidden ${type === 'image' ? 'border border-slate-200 dark:border-border-dark' : ''
                    }`}
            >
                {renderAvatarContent()}
            </div>
